...
class SponsorBlockManager {
  constructor() {
    ...
    this.cache = new Map();         // videoId -> segments[] (bounded FIFO; size <= CACHE_MAX)
  }

  // Insert with FIFO eviction so the cache can't grow unbounded over months
  // of plays. Re-insertion bumps recency by deleting then re-setting the key.
  _cacheSet(videoId, segments) {
    if (this.cache.has(videoId)) this.cache.delete(videoId);
  ...
  async onPlay(queue, song, log) {
    ...
    try {
      if (this.cache.has(videoId)) segments = this.cache.get(videoId);
        ...
        this._cacheSet(videoId, segments);
      }
    } catch (e) {
