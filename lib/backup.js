...
const create = () => {
  ...
  // v3.1.0 — include the SQLite db (base64 encoded) if it exists. This is
  // the source of truth for economy/social/sessions/history; without it the
  // snapshot is incomplete.
  try {
    const db = require('./db');
    if (db.isAvailable) {
      const dbBuf = fs.readFileSync(db.path);
      binaries['maow.db'] = dbBuf.toString('base64');
    }
  } catch (e) { /* db module missing or unavailable */ }
  const snapshot = { version: 2, createdAt: Date.now(), files, binaries };
...
const restore = (snapshotName, { dryRun = false } = {}) => {
  ...
  // v2 — also restore the SQLite db if the snapshot includes it.
  if (snap.version === 2 && snap.binaries) {
    for (const [name, b64] of Object.entries(snap.binaries)) {
      if (name === 'maow.db') {
        try {
          const db = require('./db');
          const targetPath = db.path;
          const resolvedTarget = path.resolve(targetPath);
          const resolvedRoot = path.resolve(ROOT);
          // Guard: only allow writing under ROOT (DB_PATH could be set anywhere).
          if (!resolvedTarget.startsWith(resolvedRoot + path.sep)) {
            planned.push({ rel: 'maow.db', target: targetPath, bytes: b64.length, skipped: 'db-path-outside-root' });
            continue;
          }
          planned.push({ rel: 'maow.db', target: targetPath, bytes: b64.length });
          if (!dryRun) {
            // Close the live db first so the file isn't locked.
            try { db.close(); } catch { /* */ }
          ...
          planned.push({ rel: 'maow.db', skipped: `restore failed: ${e.message}` });
        }
      }
    }
  }
