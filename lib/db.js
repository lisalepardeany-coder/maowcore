...
function runMigrations(d) {
  ...
  const migrations = [
    ...
    // v2 — dashboard user roles + posts (v3.2.0)
    {
      version: 2,
      sql: `
        -- Dashboard user roles. user_id matches Discord OAuth user IDs from
        -- the sessions table. rank is a string ladder: owner > admin > moderator
        -- > member > banned. Banned users get rejected at the auth layer.
        CREATE TABLE user_roles (
          user_id TEXT PRIMARY KEY,
          rank TEXT NOT NULL DEFAULT 'member',
          tag TEXT,             -- cached Discord tag for display (refreshed on login)
          avatar TEXT,          -- cached avatar URL
          granted_by TEXT,      -- userId that promoted them (or 'bootstrap')
          granted_at INTEGER NOT NULL,
          notes TEXT
        );
        CREATE INDEX idx_user_roles_rank ON user_roles(rank);

        -- Posts/announcements published by admins+. Markdown body.
        CREATE TABLE posts (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          title TEXT NOT NULL,
          body TEXT NOT NULL,
          category TEXT,            -- 'update' | 'announcement' | 'changelog' | 'news'
          pinned INTEGER NOT NULL DEFAULT 0,    -- 0/1 — pinned posts sort first
          author_id TEXT NOT NULL,  -- user_id that created it
          author_tag TEXT,          -- cached tag at create time
          created_at INTEGER NOT NULL,
          updated_at INTEGER NOT NULL
        );
        CREATE INDEX idx_posts_pinned_created ON posts(pinned DESC, created_at DESC);
      `,
    },
