      ...
      out.push({
        ...
        currentSong: song
          ? {
              ...
              hasSyncedLyrics: !!song.syncedLyrics?.length,
  ...
  async handleCommand(ws, raw) {
    ...
    try {
      switch (msg.action) {
        ...
        case 'loop': if (queue) queue.setRepeatMode(Number(msg.value) % 3); break;
        case 'volume':
          if (queue) queue.setVolume(Number(msg.value));
          if (guildId) updateGuild(guildId, { volume: Number(msg.value) });
          break;
        case 'seek': if (queue) await queue.seek(Number(msg.value)); break;
        case 'leave':
          if (queue) queue.stop();
        ...
        case 'queue_move': {
          if (!queue) break;
          const from = Math.max(1, Math.min(queue.songs.length - 1, Number(msg.from)));
          const to = Math.max(1, Math.min(queue.songs.length - 1, Number(msg.to)));
          if (from === to) break;
