...
class ControlServer {
  ...
  async handleHttp(req, res) {
    ...
    if (urlPath0 === '/api/library') {
      return this._json(res, { songs: library.list() });
    }
  ...
  _handleLibraryUpload(req, res) {
    ...
    if (!library.isAllowedExt(rawName)) {
      return this._json(res, { error: `Unsupported format. Allowed: ${[...library.ALLOWED_EXT].join(', ')}` }, 400);
    }
    const chunks = [];
    let total = 0;
    let aborted = false;
    req.on('data', (chunk) => {
      ...
      if (total > library.MAX_BYTES) {
        aborted = true;
        this._json(res, { error: `File too large (max ${library.MAX_BYTES / 1024 / 1024} MB).` }, 413);
        req.destroy();
        return;
      }
      chunks.push(chunk);
    ...
    req.on('end', () => {
      if (aborted) return;
      try {
        const entry = library.add(rawName, Buffer.concat(chunks));
        this.log(`⬆ Uploaded to library: ${entry.name}`);
        this._json(res, { ok: true, song: entry });
        // Probe duration in the background — fills in the manifest shortly
        // after; the dashboard picks it up on its next list refresh.
        library.probeAndStore(entry.id, path.join(library.LIB_DIR, entry.file));
      } catch (e) {
        this._json(res, { error: e.message }, 400);
      }
    });
    req.on('error', () => { if (!aborted) this._json(res, { error: 'Upload stream error' }, 500); });
  }
  ...
  // Stream an uploaded file with HTTP Range support (so the dashboard can
  // preview/seek in-browser). Path-traversal-safe: we only serve files whose
  // basename exists in the library manifest.
  _serveLibraryFile(urlPath, req, res) {
    ...
    const full = path.join(library.LIB_DIR, entry.file);
    let stat;
  ...
  // basename + existence check inside the sounds dir.
  _serveSoundFile(urlPath, req, res) {
    ...
    if (range) {
      ...
      const start = m && m[1] ? parseInt(m[1], 10) : 0;
      const end = m && m[2] ? parseInt(m[2], 10) : stat.size - 1;
      if (start >= stat.size) { res.writeHead(416, { 'Content-Range': `bytes */${stat.size}` }).end(); return; }
      res.writeHead(206, {
        'Content-Type': mime,
        'Content-Range': `bytes ${start}-${end}/${stat.size}`,
        'Accept-Ranges': 'bytes',
