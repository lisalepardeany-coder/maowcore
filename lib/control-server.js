...
class ControlServer {
  ...
  async _handleAdminMembers(req, res, url) {
    ...
    try {
      // For large guilds, fetching every member can be slow/expensive. Use the
      // cache (populated by GuildMembers intent + voice state events) and let
      // the operator request explicit fetches if needed.
  ...
  // ===== Speedtest =====
  // Spawn librespeed-cli (or speedtest-cli as fallback) if available. Cached
  // result lives in memory for the dashboard to display until a fresh run.
  async _handleAdminSpeedtest(req, res) {
    // GET → return cached result; POST → run a fresh test.
    if (req.method === 'GET') {
      return this._json(res, { ok: true, result: this._speedtestResult || null });
    }
    if (this._speedtestRunning) {
      return this._json(res, { error: 'A speedtest is already running.' }, 409);
    }
    ...
    // Try librespeed first, then speedtest-cli. Both produce JSON with --json.
    const tools = [
      { name: 'librespeed-cli', args: ['--json'] },
      { name: 'speedtest-cli', args: ['--json'] },
    ];
    const tryNext = (idx) => {
      if (idx >= tools.length) {
        this._speedtestResult = { error: 'No speedtest tool found. Install librespeed-cli or speedtest-cli.', ts: Date.now() };
        this._speedtestRunning = false;
        this.broadcast({ type: 'speedtest', result: this._speedtestResult });
        return;
      }
      const tool = tools[idx];
      const proc = spawn(tool.name, tool.args, { shell: true });
      let stdout = '', stderr = '';
      ...
      proc.on('close', (code) => {
        if (code !== 0) {
          this.log(`▲ ${tool.name} exited ${code}: ${stderr.slice(0, 200)}`, 'warn', 'system');
          return tryNext(idx + 1);
        }
        try {
          const parsed = JSON.parse(stdout);
          // Normalize: librespeed → { download, upload, ping }; speedtest-cli → { download, upload, ping }
          this._speedtestResult = {
            ts: Date.now(),
            tool: tool.name,
            // librespeed: Mbps directly; speedtest-cli: bits/sec → convert
            downloadMbps: parsed.download > 1000 ? parsed.download / 1e6 : parsed.download,
            uploadMbps: parsed.upload > 1000 ? parsed.upload / 1e6 : parsed.upload,
            pingMs: parsed.ping || parsed.latency || null,
            server: parsed.server?.name || parsed.server || null,
            isp: parsed.client?.isp || null,
          };
          this.log(`✓ Speedtest: ↓${this._speedtestResult.downloadMbps.toFixed(1)} ↑${this._speedtestResult.uploadMbps.toFixed(1)} Mbps · ${this._speedtestResult.pingMs}ms`,
            'success', 'system');
        } catch (e) {
          this._speedtestResult = { error: `Failed to parse ${tool.name} output: ${e.message}`, ts: Date.now() };
        }
        this._speedtestRunning = false;
        this.broadcast({ type: 'speedtest', result: this._speedtestResult });
      });
