...
class ControlServer {
  ...
  _checkAuth(req, res, urlPath) {
    ...
    if (urlPath === '/'
      ...
      || urlPath === '/app.js'
      || urlPath === '/style.css'
      || urlPath === '/favicon.ico') return true;
    ...
    try {
      ...
      try {
        ...
        if (roleRow?.rank === 'banned') {
          ...
          res.writeHead(302, { Location: '/dashboard.html#banned=1' }).end();
          return;
        }
      ...
      const redirect = `/dashboard.html#maow_session=${encodeURIComponent(token)}`;
  ...
  _handleEconomyLeaderboard(req, res, url) {
    ...
    const guildId = url.searchParams.get('guildId');
    if (!guildId) return this._json(res, { error: 'guildId required.' }, 400);
    return this._json(res, { leaderboard: econ.leaderboard(guildId, 20) });
  }
  _handleEconomyMe(req, res, url) {
    ...
    if (!user) return this._json(res, { error: 'not logged in' }, 401);
    if (!guildId) return this._json(res, { error: 'guildId required.' }, 400);
    return this._json(res, { user: econ.getUser(guildId, user.userId) });
  }
