...
class ControlServer {
  ...
  async _handleAuthCallback(req, res, url) {
    ...
    try {
      ...
      // hash (so it doesn't show up in server logs). The dashboard's
      // bootstrap reads location.hash, stores the token in localStorage,
      // and strips the hash from the URL.
  ...
  _handleAuthMe(req, res) {
    ...
    return this._json(res, { configured: true, user: user || null });
  }
