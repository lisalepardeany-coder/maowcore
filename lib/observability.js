...
// 10s timeout so a slow Discord webhook can't back up the alert pipeline.
...
// SET lastAlertAt UP FRONT (before the await chain) so two concurrent
// recordError calls can't both pass the cooldown check and double-fire.
const maybeAlert = () => {
  if (!ALERT_URL) return;
  ...
  // Claim the slot synchronously BEFORE kicking off async sendAlert.
  // Otherwise a second recordError on the same tick can also pass the
  // cooldown check before this one sets lastAlertAt below.
  ...
  // Fire-and-forget; don't await so we don't block the log handler.
  sendAlert({
