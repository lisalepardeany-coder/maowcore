...
const list = ({ limit = 50, offset = 0, category = null } = {}) => {
  ...
  // ORDER includes `id DESC` as a tiebreaker — multiple posts inserted in
  // the same millisecond would otherwise have indeterminate order. The
  // AUTOINCREMENT id is monotonic so this preserves "newest first" semantics
  // while making the sort stable.
  const rows = db.raw.prepare(
    `SELECT * FROM posts ${where} ORDER BY pinned DESC, created_at DESC, id DESC LIMIT ? OFFSET ?`,
