...
const startScheduler = ({ onCron, onLog } = {}) => {
  ...
  const tick = async () => {
    ...
    for (const c of state.crons) {
      ...
      try {
        const parsed = parseCron(c.expr);
        if (!cronMatches(parsed, now)) continue;
        noop(`⏰ Cron fired: ${c.name} → ${c.action.type}`, 'info');
        try { await onCron?.(c); noteCronRan(c.id, null); }
        catch (e) { noop(`✕ Cron "${c.name}" failed: ${e.message}`, 'error'); noteCronRan(c.id, e.message); }
      } catch (e) {
        noop(`✕ Cron "${c.name}" invalid expression: ${e.message}`, 'error');
      }
    }
  };
  // Tick every 20s — covers the worst-case "minute boundary missed because
  // an earlier tick was slow" case without burning cycles.
